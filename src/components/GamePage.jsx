...
import { allMembers } from '../data/gameData'; // Ensure allMembers is imported
import { baseGameQuestions } from '../data/gameData'; // Import baseGameQuestions
...
function GamePage({ questions, members, playerSelections, onSelection, currentQuestionIndex, setCurrentQuestionIndex, onGameComplete }) {
    ...
    // Filter members that have already been selected for other questions
    const usedMemberIds = playerSelections
        .filter(s => s.questionId !== currentQuestion?.id && s.selectedMemberId !== null)
        .map(s => s.selectedMemberId);

    // Filter out members that are already used in other questions
    const availableMembers = members.filter(member => !usedMemberIds.includes(member.id));

    // Effect to clear feedback when question changes
    useEffect(() => {
        setFeedbackMessage('');
    ...
    const handleMemberSelect = (memberId) => {
        if (isSelectionDisabled) return; // Prevent multiple clicks
        ...
        setFeedbackMessage(`You selected ${members.find(m => m.id === memberId)?.name}!`);
        ...
        }, 1000); // 1-second delay before moving to next question/screen
    };
    ...
    return (
        <div className="bg-gray-900 bg-opacity-80 rounded-3xl shadow-2xl p-6 md:p-10 max-w-4xl w-full text-center border-4"
            ...
            <p className="text-lg md:text-xl text-gray-200 mb-8 leading-relaxed">
                {getPieceIcon(currentQuestion.piece, succinctColors.green, '1.5em')}
                {currentQuestion.question}
            </p>

            {/* Attributes for the current piece */}
            <div className="mb-8 text-left">
                <h3 className="text-xl font-semibold mb-3" style={{ color: succinctColors.blue }}>
                    Attributes of the {currentQuestion.piece} role:
                </h3>
            ...
            <div className="pfp-grid grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6 justify-items-center">
                {availableMembers.map((member) => (
                    <div
                        ...
                        className={`pfp-card relative w-28 h-28 md:w-32 md:h-32 rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 ease-in-out
                                    ...
                                    ${selectedMemberForCurrentQuestion === member.id ? 'border-4 border-white scale-105 shadow-xl' : 'hover:scale-105 hover:shadow-xl'}
            ...
            {/* Feedback Message */}
            {feedbackMessage && (
                <p className="text-xl md:text-2xl font-bold mt-6" style={{ color: succinctColors.pink }}>
                    {feedbackMessage}
                </p>
            )}
        </div>
